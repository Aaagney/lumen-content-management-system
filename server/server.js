...
// Routes from Comment & Discussion module
  ...
  try {
    ...
    if (rows[0].count === 0) {
      for (const u of fallbackDb.users) {
        await dbConnection.query(
          `INSERT INTO users
          (id, fullname, email, password, role, bio)
          VALUES (?, ?, ?, ?, ?, ?)`,
          [
            u.id,
            u.fullname,
      ...
      for (const a of fallbackDb.articles) {
        await dbConnection.query(
          ...
          (id, title, category, read_time, views, likes, status, image_url, author_id)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [
            a.id,
            a.title,
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    const [result] = await query(
      `INSERT INTO users
      (fullname, email, password, role, bio)
      VALUES (?, ?, ?, ?, ?)`,
      [
        fullname,
        email,
...
// Get articles for User Management
